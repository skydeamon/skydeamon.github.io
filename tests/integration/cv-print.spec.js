...
test('print button on a CV page triggers window.print', async ({ page }) => {
  ...
  await page.goto('/portfolio/cv_ai_engineer.html');
  await page.click('button.control-btn.secondary');
  expect(await page.evaluate(() => window.__printed)).toBe(true);
});

test('cv_minimal.html is the ATS outlier: print-btn, no toggle, no data-theme', async ({ page }) => {
  await page.goto('/portfolio/cv_minimal.html');
